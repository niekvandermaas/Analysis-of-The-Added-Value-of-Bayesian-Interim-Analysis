######################################################################
#First run efficacy analysis HO103 seli
######################################################################
#packages
library(readxl)
library(tidyverse)
...
######################################################################
interim_list <- list(
  data_50 = hovon_103_seli[1:50,] %>% mutate(arm = droplevels(arm)), 
  data_75 = hovon_103_seli[1:75,] %>% mutate(arm = droplevels(arm)),
  data_102 = hovon_103_seli %>% mutate(arm = droplevels(arm))
)

success <- lapply(interim_list, function(df) {
  with(df, tapply(df[["early_death"]], arm, sum, na.rm = TRUE))
})
...
mcmc_samples <- lapply(data_list, function(dl) {
  ...
  update(model, 1000)
  coda.samples(model, c("diff", "theta1", "theta2", "p"), n.iter = 50000, inits = list(.RNG.name = "base::Wichmann-Hill"))
})
