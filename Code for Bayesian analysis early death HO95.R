...
set.seed(321)
######################################################################
# Bayesian analysis AE 4-5 HO95
######################################################################
interim_list <- list(
  data_1 = patient_interim_R1_599 %>%
    mutate(ed = if_else(osR1 <= 3.3 & osR1i == 1, 1, 0),
           trt = factor(armR1c2, labels = c("VMP", "HSCT"))), 
  data_2 = patient_interim_R1_898 %>%
    mutate(ed = if_else(osR1 <= 3.2854 & osR1i == 1, 1, 0),
           trt = factor(armR1c2, labels = c("VMP", "HSCT"))),
...
success <- lapply(interim_list, function(df) {
  with(df, tapply(df[["ed"]], trt, sum, na.rm = TRUE))
})
...
post %>% 
  ...
  stat_pointinterval(aes(x = value, y = 0),
                     point_interval = mode_hdi, .width = c(.95, .5)) +
  scale_x_continuous(breaks = c(0, 0.25, 0.5), labels = c("0.0%", "25.0%", "50%")) +
  scale_y_continuous(NULL, breaks = NULL) + 
  geom_text(
    data    = dat_text,
