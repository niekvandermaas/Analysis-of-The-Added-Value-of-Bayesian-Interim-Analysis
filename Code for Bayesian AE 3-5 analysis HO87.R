...
######################################################################
# Bayesian analysis AE 3-5 HO87
######################################################################
interim_list <- list(
  data_1 = ho_87_final %>%
    filter(pid %in% interim_analysis_1$pid) %>%
    mutate(ae_35 = ifelse(max(dT0) + dmonths(2) >= dstartae_35, 1, 0)), 
  data_2 = ho_87_final %>%
    filter(pid %in% interim_analysis_2$pid) %>%
    mutate(ae_35 = ifelse(max(dT0) + dmonths(2) >= dstartae_35, 1, 0)),
  data_3 = ho_87_final %>%
    filter(pid %in% interim_analysis_3$pid) %>%
    mutate(ae_35 = ifelse(max(dT0) + dmonths(2) >= dstartae_35, 1, 0))
)
...
post %>% 
  ...
  stat_pointinterval(aes(x = value, y = 0),
                     point_interval = mode_hdi, .width = c(.95, .5)) +
  scale_x_continuous(breaks = c(0, 0.10, 0.20), labels = c("0.0%", "10.0%", "20.0%")) +
  scale_y_continuous(NULL, breaks = NULL) + 
  geom_text(
    data    = dat_text,
